...
async function printCpuTemperature() {
    const data = await si.cpuTemperature();
    return `CPU Temperature: ${!data.main ? "not supported" : data.main}`;
}
async function printGraphicsControllers() {
...
async function printMemoryUsage() {
    ...
    return `Total Memory: ${totalMemory.toFixed(2)} GB\nUsed Memory: ${usedMemory.toFixed(2)} GB\nFree Memory: ${freeMemory.toFixed(2)} GB`;
}
async function printBatteryStatus() {
    const data = await si.battery();
    return `Battery Status: ${data.isCharging ? "Charging" : "Not Charging"}\nBattery Percent: ${data.percent}%\nBattery Time Remaining: ${data.isCharging ? "infinite" : data.timeRemaining} minutes`;
}
const printFunctions = await Promise.all([
    printOS(),
    printArchitecture(),
    printUserName(),
    printCpuCores(),
